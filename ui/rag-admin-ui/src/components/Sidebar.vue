  ...
  <aside class="rag-card flex flex-col h-full">
    ...
    <div class="mb-4">
      <div class="flex w-full items-stretch gap-2">
        <input v-model="newWorkspaceId" @keyup.enter="useNewWorkspace"
          ...
          type="text" placeholder="New workspace id..." />
        <button type="button" @click="useNewWorkspace"
          ...
          aria-label="Use new workspace" title="Use">
          Use
        </button>
      </div>
    </div>
    ...
    <div class="mt-3">
      ...
      <ul class="space-y-2">
        <li v-for="workspace in workspaces" :key="workspace" class="w-full"
          :class="{ active: selectedWorkspace === workspace }">
          <div class="flex items-center gap-2">
            <button type="button" @click="handleSelection(workspace)" class="flex-1 text-left px-3 py-2 rounded-lg text-sm font-medium truncate
           bg-slate-900/60 text-slate-200 hover:bg-slate-800
           border border-transparent transition" :class="{
            ...
            <button type="button"
              class="shrink-0 rounded-lg border border-slate-700 bg-slate-900 px-2 py-2 text-xs text-slate-300 hover:bg-slate-800"
              title="Delete workspace" @click.stop="requestDelete(workspace)">
              Delete
            </button>
          </div>
