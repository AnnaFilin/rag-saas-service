...
<template>
  <div
    :class="['rag-card-soft cursor-pointer border transition-colors', isSelected ? 'border-emerald-400' : 'border-transparent hover:border-emerald-400']">
    <div class="flex items-start gap-2">
      ...
      <button type="button" class="mt-1 rounded-md px-2 py-1 text-xs text-slate-400 hover:text-white hover:bg-slate-800"
        title="Delete note" @click.stop="emit('delete', item.id)">
        Delete
      </button>
    </div>
