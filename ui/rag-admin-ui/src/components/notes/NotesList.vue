...
<template>
  <div class="flex flex-col gap-4 min-h-0">
    <div v-if="isLoading" class="flex items-center gap-3 text-xs text-slate-500">
      <div class="h-4 w-4 animate-spin rounded-full border-2 border-slate-700 border-t-slate-300" aria-label="Loading">
      </div>
    ...
    <template v-else>
      <div v-for="item in notes" :key="item.id">
        <NoteListItem :item="item" :isSelected="selectedId === item.id" @select="emit('select', $event)"
