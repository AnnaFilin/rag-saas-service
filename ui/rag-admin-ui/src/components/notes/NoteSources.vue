    ...
    <div class="mt-4">
        ...
        <template v-if="groupedSources.length">
            <div class="mt-2 flex flex-col gap-2">
                <div v-for="group in groupedSources" :key="group.key"
                    class="rounded-lg border border-slate-800 bg-slate-900/70 p-3">
                    <button type="button" class="flex w-full items-center gap-3 text-sm font-medium text-slate-100"
                    ...
                    <div v-if="isGroupExpanded(group.key)" class="mt-2 space-y-2 text-[0.7rem] text-slate-300 pr-1">
                        <div v-for="entry in group.entries" :key="entry"
                            ...
                            <button v-if="entry && entry.length > 240" type="button"
